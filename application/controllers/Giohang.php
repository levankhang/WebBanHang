...
class Giohang extends CI_Controller {
    ...
    public function checkout(){
        if($this->session->userdata('sessionChayNgayDiStore')){
    ...
    public function info_order(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($this->session->userdata('info-customer')){
                ...
                if($this->session->userdata('sessionChayNgayDiStore')){
                    $info=$this->session->userdata('sessionChayNgayDiStore');
                    $idCustomer=$info['id'];
                }
            }
            ...
            $config['smtp_user']    = 'levankhang197@gmail.com';
            $config['smtp_pass']    = 'Mat khau gmail...';
            ...
            $this->email->from('levankhang197@gmail.com', 'Chạy Ngay Đi Store');
            $list = array('levankhang197@gmail.com');//Mặc định là email của admin
            $this->email->to($list);
            $this->email->subject('Hệ thống Chạy Ngay Đi Store');
    ...
    public function thankyou(){
        if($this->session->userdata('info-customer')||$this->session->userdata('sessionChayNgayDiStore')){
            if($this->session->userdata('sessionChayNgayDiStore')){
                $val = $this->session->userdata('sessionChayNgayDiStore');
            }else{
        ...
        $this->data['title']='Kết quả đơn hàng';
